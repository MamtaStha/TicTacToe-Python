...
def draw_player(window, grid, board, locked_positions, mouse_position, draw_object):
    query_field = 0
...
def draw_ai(window, grid, x, y, AI_object, board, locked_positions):
    """
    ...
    draw_O(window, field)
    next_object = "cross"
    update_board(board, locked_positions, index, 2)
    return next_object
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if (
                ...
                and check_all_positions(game_board)
            ):
                turn_text(game_window, game_board, game_font, current_object, winner)
                position = pygame.mouse.get_pos()

                if not winner:
                    current_object = draw_player(
                        game_window,
                        game_grid,
                        game_board,
